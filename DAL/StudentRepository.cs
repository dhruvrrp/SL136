namespace Repository
{
    ...
    public class StudentRepository : BaseRepository, IStudentRepository
    {
        ...
        public List<string> CalculateGPA(string studentId, ref List<string> errors)
        {
            var conn = new SqlConnection(ConnectionString);
            ...
            try
            {
                var adapter = new SqlDataAdapter(CalculateStudentGPA, conn)
                {
                    SelectCommand =
                    {
                        CommandType = CommandType.StoredProcedure
                ...
                for (var i = 0; i < dataSet.Tables[0].Rows.Count; i++)
                {
                    gradeList.Add(dataSet.Tables[0].Rows[i]["grade"].ToString());
                }
            }
            catch (Exception e)
            {
